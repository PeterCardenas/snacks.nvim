...
---@field _fallback? table<number, number>
local M = {}
M.__index = M
...
---@return number
function M:_next_fallback_pid()
  fallback_pid = fallback_pid + 1
  return fallback_pid
end

function M:_clear_fallback()
  if not self._fallback then
    return
  end
  for _, pid in pairs(self._fallback) do
    terminal.request({ a = "d", d = "i", i = self.img.id, p = pid })
  end
  self._fallback = {}
end
...
function M:_fallback_cursor(win)
  if self.opts.inline then
    ...
    -- Virtual line mode renders below the source range.
    if self.opts.render_mode == "virt_lines" then
      row = row + 1
      col = 0
    end
    local ok, screen = pcall(vim.fn.screenpos, win, row, col + 1)
    if ok and screen and screen.row > 0 and screen.col > 0 then
      return { screen.row, screen.col - 1 }
    end
  end
  ...
  then
    return { pos[1] + border.top, pos[2] + border.left }
  end
  return { pos[1] + 1 + border.top, pos[2] + border.left }
end
...
function M:render_fallback(state)
  ...
  local current = {} ---@type table<number, true>
  for _, win in ipairs(state.wins) do
    current[win] = true
  end
  for win, pid in pairs(self._fallback) do
    if not current[win] then
      terminal.request({ a = "d", d = "i", i = self.img.id, p = pid })
      self._fallback[win] = nil
    end
  end
  for _, win in ipairs(state.wins) do
    local pid = self._fallback[win]
    if not pid then
      pid = self:_next_fallback_pid()
      self._fallback[win] = pid
    end
    self:debug("render_fallback", win, pid)
    terminal.set_cursor(assert(self:_fallback_cursor(win)))
    terminal.request({
      a = "p",
      i = self.img.id,
      p = pid,
